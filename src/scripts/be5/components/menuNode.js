...
const MenuNode = React.createClass({
  ...
  componentDidMount() {
    ...
    if (hasAction) {
      ...
      classes += ' menuItemWithoutRef active';
    }
    this.setState({href: href, target: target, classes: classes});
  },
  ...
  _getHead() {
    return (
      React.DOM.a({href: this.state.href, className: this.state.classes, target: this.state.target, onClick: this._onClick, key: 'a ' + this.props.data.title}, this.props.data.title)
    );
  },
